from httpx import ASGITransport, AsyncClient

from app.clients.riot import get_riot_client
from app.main import app


class FakeRiotClient:
    def __init__(self) -> None:
        self.status_calls: list[str] = []

    async def get_lol_status(self, platform: str) -> dict:
        self.status_calls.append(platform)
        return {
            "id": platform,
            "name": "Oceania",
            "locales": ["en_AU"],
            "maintenances": [],
            "incidents": [],
        }


async def test_lol_status_route_accepts_explicit_platform_region() -> None:
    fake_client = FakeRiotClient()

    async def fake_riot_client() -> FakeRiotClient:
        return fake_client

    app.dependency_overrides[get_riot_client] = fake_riot_client
    transport = ASGITransport(app=app)

    try:
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.get("/api/v1/status/OC1")
    finally:
        app.dependency_overrides.clear()

    assert response.status_code == 200
    assert response.json() == {
        "id": "oc1",
        "name": "Oceania",
        "locales": ["en_AU"],
        "maintenances": [],
        "incidents": [],
    }
    assert fake_client.status_calls == ["oc1"]


async def test_lol_status_route_rejects_routing_region_as_platform() -> None:
    async def fake_riot_client() -> object:
        return object()

    app.dependency_overrides[get_riot_client] = fake_riot_client
    transport = ASGITransport(app=app)

    try:
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.get("/api/v1/status/SEA")
    finally:
        app.dependency_overrides.clear()

    assert response.status_code == 422
    assert response.json()["detail"]["code"] == "INVALID_REGION"
